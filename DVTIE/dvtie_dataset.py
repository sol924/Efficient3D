...
class DVTIEDataset(Dataset):
    ...
    def __init__(
        ...
        feat_file="/gpfs/work/cpt/yuhuilin21/PointLLM/dataset/annotations/scannet_mask3d_uni3d_feats.pt",
        img_feat_file="/gpfs/work/cpt/yuhuilin21/PointLLM/dataset/annotations/scannet_mask3d_videofeats.pt",
        max_obj_num=100,
        feat_dim=1024,
        img_feat_dim=1024,
    ):
        ...
        if DVTIEDataset.cached_feats is not None:
            self.scene_features_dict = DVTIEDataset.cached_feats
        else:
            self.scene_features_dict = self.prepare_scene_features()
            DVTIEDataset.cached_feats = self.scene_features_dict
    ...
    def __getitem__(self, index):
        ...
        # attn_map = self.attn_maps[self.annotations[index]["index"]]["a_self_plus_all"]
        # attn_map = attn_map[:, 2:, :]  # (1, 30, 100)
        # attn_map = attn_map.mean(dim=1, keepdim=True)
        # attn_map = attn_map.squeeze()  # 或者 attn_map = attn_map.view(-1)
