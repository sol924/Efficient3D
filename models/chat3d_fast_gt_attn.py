        ...
        if not self.debug:
            ...
            if self.low_resource:
                self.llama_model = LlamaForCausalLM.from_pretrained(
                    ...
                    load_in_8bit=True,
                    device_map="auto",
                    attn_implementation="flash_attention_2",  # ⚠️ 若要用自适应，请改成默认/SDPA，并确保能返回attention
                )
                # --- 新增：自适应参数 ---
                self.llama_model.config.use_adaptive = True
                self.llama_model.config.adaptive_margin = 0.0
                self.llama_model.config.adaptive_denominator = 30
                use_fast_v = bool(self.config.use_fast_v) and self.token_pruning
                use_fast_v_oracle = (
                    bool(self.config.use_fast_v_oracle) and self.token_pruning
                )
                # fast_v 保持原样（若你也需要）
                ...
                self.llama_model.config.fast_v_attention_rank = self.config.fast_v_attention_rank
                ...
                # 同步到内部模型（包括我们新增的 use_adaptive 三项）
                ...
                self.llama_model = LlamaForCausalLM.from_pretrained(
                    llama_model_path,
                    torch_dtype=torch.bfloat16,
                    # attn_implementation="flash_attention_2",  # 自适应需要attention weights，建议别开FA2
                    use_cache=False,
                    output_attentions=True,
                )
            # # # --- 你原有的 fast_v 配置 ---
            # self.llama_model.config.use_fast_v = self.config.use_fast_v
            # self.llama_model.config.fast_v_sys_length = 45
            # self.llama_model.config.fast_v_image_token_length = 300
            # self.llama_model.config.fast_v_attention_rank = self.config.fast_v_attention_rank
            # self.llama_model.config.fast_v_agg_layer = self.config.fast_v_agg_layer
            # self.llama_model.config.use_fast_v_oracle = self.config.use_fast_v_oracle
            # # --- 新增：自适应参数 ---
            # self.llama_model.config.use_adaptive = True
            # self.llama_model.config.adaptive_margin = -0.02  # 你的 B
            # self.llama_model.config.adaptive_denominator = 30  # 每层/30 累计
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        for i in range(batch_size):
            ...
            a_map = attention_maps[i].unsqueeze(-1).repeat(1, 3).reshape(-1).unsqueeze(0)
            with self.maybe_autocast():
            ...
            output_text = output_text.split(self.start_sym)[-1]
