    ...
    def evaluate(
        ...
        **kwargs,
        ...
        for i in range(batch_size):
            ...
            if self.config.adaptive_rank_alpha > 0:
                adaptive_rank = find_min_rank_sum_larger_than_alpha(
                    attention_maps[i].softmax(-1), self.config.adaptive_rank_alpha
                )
                adaptive_rank = adaptive_rank * 3
            else:
                adaptive_rank = self.config.fast_v_attention_rank
            ## set config
            # self.llama_model.config.use_fast_v_oracle = True
            self.llama_model.config.fast_v_attention_rank = adaptive_rank
...
def find_min_rank_sum_larger_than_alpha(attn_map, alpha=0.95):
    ...
    找到最小的rank，使得top rank 的attention sum 大于等于alpha
    """
