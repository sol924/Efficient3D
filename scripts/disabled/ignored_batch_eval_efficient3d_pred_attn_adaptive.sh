...
# Legacy adaptive runner kept under disabled/ for reference.
# Use scripts/batch_eval_efficient3d_pred_attn_adaptive.sh in normal workflow.
...
use_a_map_ori=False
val_attn_maps_path="/gpfs/work/cpt/yuhuilin21/PointLLM/dataset/pred_attn_maps"
alpha_list=(0.21)
Ks=(0)
...
for ((i=0; i<len; i++)); do
  ...
  other_info="efficient3d_pred_attn_ada_${use_external_attn_maps}_layer${k}_alpha${alpha}_oracle${use_fast_v_oracle}_a_map_ori${use_a_map_ori}"
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node="${gpu_num}" \
    tasks/inference_efficient3d_pred_attn_ada.py \
