    ...
    def __getitem__(self, index):
        ...
        if self.attn_maps is not None:
            if self.config.use_a_map_ori:
                ...
                attn_map = self.attn_maps[self.anno[index]['index']]['a_map']
            # attn_map = self.attn_maps[self.anno[index]['index']]['a_map']  # shape: (1, 32, 100)
            # attn_map = attn_map[:, 2:, :]
            # # attn_map_first3 = attn_map[:, :21, :]
            # attn_map_mean = attn_map.mean(dim=1, keepdim=True)
            # attn_map = attn_map_mean.squeeze()
